...
if menu == "Prediction":
    ...
    with st.expander("👋 Selamat Datang! Klik di sini untuk melihat Alur Kerja Aplikasi", expanded=True):
        st.markdown(
            """            
            1.  **Menu Prediction (Halaman ini):**
                * Unggah file mentah Anda (.csv/.xlsx) di bagian **"Prediksi Batch"** di bawah.
                * Model akan memprediksi kolom `topic` dan `type` (intensi).
                * **Unduh file hasil prediksi** tersebut.

            2.  **Menu Dataset / Dashboard:**
                * Unggah file hasil (yang sudah ber-label) pada **sidebar di kiri**.
                * Data Anda akan otomatis ditampilkan di tabel (menu Dataset) atau dashboard visual (menu Dashboard).
            
            ---
            *Note: Jika Anda sudah memiliki file yang berisi `topic` & `type`, Anda bisa langsung ke menu Dataset atau Dashboard dan unggah file Anda di sidebar.*
            """
        )
            ...
            if st.button("Mulai Prediksi Batch"):
                with st.spinner(f"Memproses {len(df_batch):,} baris data... Ini mungkin perlu waktu."):
                    try:
                        ...
                        st.success("🎉 **Prediksi Selesai!** File Anda siap diunduh.")
                        st.info("💡 **Langkah Selanjutnya:** Unggah file yang baru saja Anda unduh pada sidebar (di menu 'Dataset' atau 'Dashboard') untuk divisualisasikan.")
    ...
    if df is not None:
        ...
        st.info(
            """
            ### Unggah File Anda untuk Memulai
            
            Silakan unggah file data (.csv/.xlsx) Anda pada **sidebar di sebelah kiri** untuk melihat datanya di sini.
            
            ---
            
            💡 **Belum punya file data yang ber-label (`topic` & `type`)?**
            
            1.  Buka menu **"Prediction"** (menu pertama).
            2.  Unggah file mentah Anda di bagian "Prediksi Batch".
            3.  Unduh file hasil prediksi yang sudah berisi kolom `topic` dan `type`.
            4.  Kembali ke halaman ini dan unggah file baru tersebut di sidebar.
            """
        )
    ...
    if df is not None:
        ...
        st.info(
            ...
            Silakan unggah file data (.csv/.xlsx) Anda pada **sidebar di sebelah kiri** untuk melihat dashboard visual.
            ...
            1.  Buka menu **"Prediction"** (menu pertama).
            2.  Unggah file mentah Anda dan unduh hasilnya.
            3.  Kembali ke halaman ini dan unggah file baru tersebut di sidebar.
            """
        )
    ...
    tab_info, tab_eval, tab_manual = st.tabs([
    ...
    with tab_eval:
        ...
        st.markdown("## Evaluasi Model Teknis") 
        st.info("Halaman ini berisi data teknis hasil evaluasi model yang digunakan dalam aplikasi ini, ditujukan untuk developer dan akademisi.")
        ...
        with filter_c1: model_choice = st.selectbox("Pilih Model:", ["Topic", "Intention"], key="eval_model_choice")
        with filter_c2: experiment_type = st.selectbox("Pilih Tipe Eksperimen:",["Parameter Default", "Hyperparameter Tuned"], key="eval_exp_type")
        with filter_c3: split = st.selectbox("Pilih Split Data:", ["70/30", "80/20"], key="eval_split")
        ...
        st.download_button("⬇️ Download Report",csv,file_name=f"report_{model_choice}_{experiment_type.replace(' ','_')}_{split.replace('/','-')}.csv",mime="text/csv", key="eval_download_csv")
    ...
    with tab_manual:
        ...
        st.markdown("Klik tombol di bawah ini untuk mengunduh buku manual lengkap (file PDF) yang menjelaskan cara penggunaan setiap fitur di dashboard ini.")
        st.markdown("") 
        
        pdf_raw_link = "https://github.com/kuuntakiintee/Dashboard-Klasifikasi-Tweet-Gojek-Grab/raw/main/535220129_Buku%20Manual.pdf"
        
        download_filename = "Buku_Manual_Dashboard_Analisis.pdf"
        
        st.markdown(
            f'<a href="{pdf_raw_link}" download="{download_filename}" class="download-button">Download Buku Manual 📖</a>',
            unsafe_allow_html=True
        )
